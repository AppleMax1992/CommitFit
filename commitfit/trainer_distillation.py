...
from . import CommitFitTrainer, logging
...
class DistillationCommitFitTrainer(CommitFitTrainer):
    """Trainer to compress a CommitFit model with knowledge distillation.

    Args:
        teacher_model (`CommitFitModel`):
            The teacher model to mimic.
        train_dataset (`Dataset`):
            The training dataset.
        student_model (`CommitFitModel`):
        ...
        model_init (`Callable[[], CommitFitModel]`, *optional*):
            A function that instantiates the model to be used. If provided, each call to [`~DistillationCommitFitTrainer.train`] will start
    ...
    def __init__(
        self,
        teacher_model: "CommitFitModel",
        student_model: Optional["CommitFitModel"] = None,
        ...
        super(DistillationCommitFitTrainer, self).__init__(
            model=student_model,
    ...
    def train(
        ...
        if self.train_dataset is None:
            raise ValueError(
                "Training requires a `train_dataset` given to the `DistillationCommitFitTrainer` initialization."
            )
