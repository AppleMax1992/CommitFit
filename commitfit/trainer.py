...
if TYPE_CHECKING:
    ...
    from .modeling import CommitFitModel

logging.set_verbosity_info()
logger = logging.get_logger(__name__)


class CommitFitTrainer:
    """Trainer to train a CommitFit model.

    Args:
        model (`CommitFitModel`, *optional*):
            The model to train. If not provided, a `model_init` must be passed.
        train_dataset (`Dataset`):
            The training dataset.
        train_change_code (`nparray`, *optional*):
            The feature from train code change.
        eval_dataset (`Dataset`, *optional*):
            The evaluation dataset.
        test_change_code (`nparray`, *optional*):
            The feature from test code change.
        model_init (`Callable[[], CommitFitModel]`, *optional*):
            A function that instantiates the model to be used. If provided, each call to [`~CommitFitTrainer.train`] will start
    ...
    def __init__(
        self,
        model: Optional["CommitFitModel"] = None,
        train_dataset: Optional["Dataset"] = None,
        train_code_change: Optional[list] = None,
        test_code_change: Optional[list] = None,
        eval_dataset: Optional["Dataset"] = None,
        model_init: Optional[Callable[[], "CommitFitModel"]] = None,
        ...
        self.train_code_change = train_code_change
        self.eval_dataset = eval_dataset
        self.test_code_change = test_code_change
        ...
        if model is None:
            if model_init is not None:
                ...
                raise RuntimeError("`CommitFitTrainer` requires either a `model` or `model_init` argument")
        else:
            if model_init is not None:
                raise RuntimeError("`CommitFitTrainer` requires either a `model` or `model_init` argument, but not both")
    ...
    def _validate_column_mapping(self, dataset: "Dataset") -> None:
        ...
        if self.column_mapping is None and not required_columns.issubset(column_names):
            ...
            if column_names == {"train"} and isinstance(dataset, DatasetDict):
                raise ValueError(
                    "CommitFit expected a Dataset, but it got a DatasetDict with the split ['train']. "
                    "Did you mean to select the training split with dataset['train']?"
                )
            elif isinstance(dataset, DatasetDict):
                raise ValueError(
                    f"CommitFit expected a Dataset, but it got a DatasetDict with the splits {sorted(column_names)}. "
                    "Did you mean to select one of these splits from the dataset?"
                )
            else:
                raise ValueError(
                    f"CommitFit expected the dataset to have the columns {sorted(required_columns)}, "
                    f"but only the columns {sorted(column_names)} were found. "
                    "Either make sure these columns are present, or specify which columns to use with column_mapping in CommitFitTrainer."
                )
        ...
        for key, value in params.items():
            if hasattr(self, key):
                ...
                logger.warning(
                    ...
                    "`CommitFitTrainer`, and `model_init` does not take any arguments."
                )
    ...
    def freeze(self):
        """
        Freeze CommitFitModel's differentiable head.
        Note: call this function only when using the differentiable head.
        """
        if not self.model.has_differentiable_head:
            raise ValueError("Please use the differentiable head in `CommitFitModel` when calling this function.")
    ...
    def unfreeze(self, keep_body_frozen: bool = False):
        """
        Unfreeze CommitFitModel's differentiable head.
        Note: call this function only when using the differentiable head.
    ...
    def train(
        ...
        Args:
            ...
            learning_rate (`float`, *optional*):
                Temporary change the learning rate to use for contrastive training or CommitFitModel's head in logistic regression.
                If ignore, will use the value given in initialization.
            body_learning_rate (`float`, *optional*):
                Temporary change the learning rate to use for CommitFitModel's body in logistic regression only.
                If ignore, will be the same as `learning_rate`.
            l2_weight (`float`, *optional*):
                Temporary change the weight of L2 regularization for CommitFitModel's differentiable head in logistic regression.
        ...
        if self.train_dataset is None:
            raise ValueError("Training requires a `train_dataset` given to the `CommitFitTrainer` initialization.")
        ...
        train_code_change = self.train_code_change
    ...
    def evaluate(self):
        ...
        test_code_change = self.test_code_change
        y_test = eval_dataset["label"]

        logger.info("***** Running evaluation *****")
        y_pred = self.model.predict(x_test,test_code_change)
    ...
    def hyperparameter_search(
        ...
        **kwargs,
        ...
        To use this method, you need to have provided a `model_init` when initializing your [`CommitFitTrainer`]: we need to
    ...
    def push_to_hub(self, repo_id: str, **kwargs) -> str:
        ...
        Args:
            repo_id (`str`):
                The full repository ID to push to, e.g. `"tomaarsen/CommitFit_sst2"`.
        ...
        if "/" not in repo_id:
            raise ValueError(
                '`repo_id` must be a full repository ID, including organisation, e.g. "tomaarsen/CommitFit_sst2".'
            )
        commit_message = kwargs.pop("commit_message", "Add CommitFit model")
