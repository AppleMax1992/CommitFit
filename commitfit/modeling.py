...
from . import logging
from .data import CommitFitDataset
...
class CommitFitHead(models.Dense):
    """
    A CommitFit head that supports multi-class classification for end-to-end training.
    ...
    Args:
        in_features (`int`, *optional*):
            The embedding dimension from the output of the CommitFit body. If `None`, defaults to `LazyLinear`.
    ...
    def forward(
        ...
        """
        CommitFitHead can accept embeddings in:
    ...
    def __repr__(self):
        return "CommitFitHead({})".format(self.get_config_dict())


@dataclass
class CommitFitModel(PyTorchModelHubMixin):
    """A CommitFit model with integration to the Hugging Face Hub."""

    def __init__(
        self,
        model_body: Optional[SentenceTransformer] = None,
        model_head: Optional[Union[CommitFitHead, LogisticRegression]] = None,
        multi_target_strategy: Optional[str] = None,
        l2_weight: float = 1e-2,
        normalize_embeddings: bool = False,
    ) -> None:
        super(CommitFitModel, self).__init__()
    ...
    def fit(
        self,
        x_train: List[str],
        train_code_change:  Union[List[str], np.ndarray],
        ...
        if self.has_differentiable_head:  # train with pyTorch
            ...
            feature_type = type(train_code_change)
            embeddings = self.model_body.encode(x_train, normalize_embeddings=self.normalize_embeddings)
            # If train_code_change is float64 that needed to embed
            if((train_code_change is not None) and feature_type ==  np.ndarray):
                code_embeddings = train_code_change
                x_e, y_e = embeddings.shape
                x_b, y_b = code_embeddings.shape
                # Define the shape of the needed to fill train samples
                # 90 is the code change feature numbers
                train_need2fill_shape = (x_e - x_b , y_b) 
                np_train = np.zeros(train_need2fill_shape)
                # 给生成的sentence 补齐 code change
                filled_code_embeddings = np.concatenate((code_embeddings, np_train), axis=0)
                final_embeddings = np.concatenate((embeddings, filled_code_embeddings), axis=1)
            # If train_code_change is float64 that don't need to embed, can directly to concat 
            elif(feature_type == list):
                code_embeddings = self.model_body.encode(train_code_change, normalize_embeddings=self.normalize_embeddings)
                x_e, y_e = embeddings.shape
                x_b, y_b = code_embeddings.shape
                # Define the shape of the needed to fill train samples
                # 90 is the code change feature numbers
            ...
            self.model_head.fit(final_embeddings, y_train)
    ...
    def predict(self, x_test: List[str], test_code_change: Union[List[str], np.ndarray] = None, as_numpy: bool = False) -> Union[torch.Tensor, "ndarray"]:
        ...
        if((test_code_change is not None) and (feature_type == np.ndarray)):
            ...
            final_embeddings = embeddings
        outputs = self.model_head.predict(final_embeddings)
    ...
    def to(self, device: Union[str, torch.device]) -> "CommitFitModel":
        """Move this CommitFitModel to `device`, and then return `self`. This method does not copy.

        Args:
            device (Union[str, torch.device]): The identifier of the device to move the model to.

        Returns:
            CommitFitModel: Returns the original model, but now on the desired device.
        """
        ...
        **model_kwargs,
        ...
        if model_head_file is not None:
            ...
            if use_differentiable_head:
                ...
                model_head = CommitFitHead(**{**head_params, **base_head_params})
            else:
...
class SKLearnWrapper:
    ...
    def save(self, path):
        ...
        joblib.dump(self.clf, f"{path}/CommitFit_head.pkl")

    def load(self, path):
        self.st_model = SentenceTransformer(model_name_or_path=path)
        self.clf = joblib.load(f"{path}/CommitFit_head.pkl")
